...
state (a cell gets vegetated if $Q <= D$ where $D$ is the vegetation density and
$Q$ is a random number between 0 and 1). The entire bottom row will be set on
fire initially (regardless of the content of the cells). The simulation runs
until there are no more burning cells.
...
\begin{itemize}
    \item Whether the top of the field had been reached.
...
boundary (45) at a density of $0.5$.
...
other side in every trial, while the number of steps required to do so is rather
consistent past the $0.6$ mark.
